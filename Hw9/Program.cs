using System;
using System.Linq;
using Xrm.ReportUtility.ChainOfResponsibility;
...
namespace Xrm.ReportUtility
{
    public static class Program
    {
        ...
        {
            var arg = new[] {"Files/table.csv", "-data", "-weightSum", "-costSum"};
            //var service = GetReportService(new[]{"Files/table.txt", "-data", "-weightSum", "-costSum"});
            var service = new GetReportService();

            var report = service.Validate(arg).CreateReport();
            //report = new DecoratorBuilder(report)
